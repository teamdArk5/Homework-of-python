...
def check(ip, port=445):
    ...
    try:
        ...
        if nt_status == '\x05\x02\x00\xc0':
            ...
            if "2008" in native_os or "7" in native_os:
                with open('vul.txt','a') as succ:
                    succ.write(ip+'\n')
...
def getips():
    ...
    lines = f.readlines()
    for line in lines:
        getip.append(line.strip())
    f.close()
    return getip
if __name__ == '__main__':
    import sys
    if len(sys.argv) != 2:
        print("{} <ip_file>".format(sys.argv[0]))
        sys.exit(1)
    else:
        ips = getips()
        pool = threadpool.ThreadPool(30)    
        requestsx = threadpool.makeRequests(check, ips)    
